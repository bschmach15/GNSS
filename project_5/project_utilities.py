...
def parse_rinex_obs_file(filepath):
    '''Given the filepath to a RINEX v2.10 observation file, parses and returns header
    ...
    Output
    ------
    `header, observations` where `header` is a namespace containing the parsed header information
        and `obs_data` is a namespace containing the observation data in the format:
        {<sat_id>: namespace(time=array, signals={<sig_id>: namespace(<obs_name>=array)})}
        
        Notes:
        - `time` on the satellite namespace is a `numpy.datetime64` object
        - <sat_id> is the IGS satellite ID -- for GPS, it is just 'G<PRN>',
            e.g. PRN 1 would be 'G01'
        - <sig_id> is 'L1' for GPS L1 and 'L2' for GPS L2
        - <obs_name> is 'pr' for code pseudorange in meters, 'carrier' for carrier phase in cycles
            'doppler' for Doppler frequency in cycles/second, 'snr' for signal-to-noise ratio in decibels

        Example usage for PRN 21:
            ```
            satellite = observations['G21']
            signal = satellite.signals['L1']
            time = satellite.time
